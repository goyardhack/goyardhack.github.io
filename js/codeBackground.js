(function () {
  ...
  function buildColumns() {
    ...
    const colCount = Math.max(isMobile ? 2 : 3, Math.floor(width / colWidth));
  ...
  function resize(forceLayout) {
    const nextWidth = Math.round(getViewportWidth());
    const nextHeight = Math.round(getViewportHeight());
    const widthChanged = Math.abs(nextWidth - width) > 1;

    if (!forceLayout && isMobile && !widthChanged) {
      return;
    }
    ...
    width = nextWidth;
    height = isMobile && lockedHeight > 0 ? lockedHeight : nextHeight;

    canvas.width = width;
    canvas.height = height;

    fontSize = Math.max(11, Math.min(14, width / (isMobile ? 70 : 90)));
  ...
  function draw() {
    ...
    const spawnRate = isMobile ? 1.8 : 1.2;

    columns.forEach((col) => {
      col.nextSpawn -= spawnRate;
      if (col.nextSpawn <= 0) {
        spawnLine(col);
